...
export function Room() {
    ...
    async function handleLikeQuestion(questionId: string, likeId: string | undefined) {
        ...
        if (likeId) {
            ...
            // se já existir um like na pergunta, remove o like que já foi dado
        }
        else {
                ...
                {questions.map(question => {
                    return (
                        <Question key={question.id} content={question.content} author={question.author} isAnswered={question.isAnswered} isHighlighted={question.isHighlighted}>
                            {/* se a pergunta não estiver respondida, o botão de like é exibido. senão, ele não é exibido */}
                            {!question.isAnswered && (
                                <>
                                <button className={`like-button ${question.likeId ? 'liked' : ''}`} type="button" aria-label="Marcar como gostei" onClick={() => handleLikeQuestion(question.id, question.likeId)}>
                                    ...
                                    {/* se o número de likes for maior que 0, exibirá o contador */}
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        stroke="#fefefe" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                                    </svg>
                                </button>
                                </>
                            )}
